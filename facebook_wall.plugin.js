(function($) {
	$.fn.facebook_wall = function(options) {
		...
		options = $.extend({
			...
			text_labels: {
				shares: {
					singular: 'Compartido % vez',
					plural: 'Compartido % veces'
				},
				likes: {
					singular: '% Me gusta',
					plural: '% Me gusta'
		...
		function timeToHuman(time) {
			...
			if (time_difference < 10) {
				...
				return Math.round(time_difference) + ' segundos';
			} else if (Math.round(time_difference/60) === 1) {
				return Math.round(time_difference/60) + ' minutos';
			} else if (Math.round(time_difference/60) < 60) {
				return Math.round(time_difference/60) + ' minutos';
			} else if (Math.round(time_difference/(60*60)) === 1) {
				return Math.round(time_difference/(60*60)) + ' hora';
			} else if (Math.round(time_difference/(60*60)) < 24) {
				return Math.round(time_difference/(60*60)) + ' horas';
			} else if (Math.round(time_difference/(60*60*24)) === 1) {
				return Math.round(time_difference/(60*60*24)) + ' dia';
			} else if (Math.round(time_difference/(60*60*24)) <= 10) {
				return Math.round(time_difference/(60*60*24)) + ' dias';
			} else {
